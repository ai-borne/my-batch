...
describe('UX-3 private directory callables', () => {
  it('returns only bounded redacted directory projections with stable cursors and house counts', async () => {
    ...
    const first = await caller.call<{ members: Array<{ uid: string; displayName: string; privateAbout?: string }>; nextCursor: { displayName: string; uid: string } | null; hasMore: boolean }>('listDirectoryMembers', { batchId, limit: 2 })
    expect(first.data.members).toEqual([{ uid: 'a', displayName: 'Aman', houseId: 'tilak', city: 'Pune', profession: 'Engineer', avatarPath: null }, { uid: 'b', displayName: 'Bharat', houseId: 'tilak', city: 'Pune', profession: 'Engineer', avatarPath: null }])
    expect(first.data.members[0]).not.toHaveProperty('privateAbout'); expect(first.data.hasMore).toBe(true)
    const second = await caller.call<{ members: Array<{ uid: string }> }>('listDirectoryMembers', { batchId, limit: 2, cursor: first.data.nextCursor })
    expect(second.data.members).toEqual([expect.objectContaining({ uid: 'c' })])
    const houses = await caller.call<{ houses: Array<{ id: string; memberCount: number }> }>('listDirectoryHouses', { batchId })
    expect(houses.data.houses).toEqual(expect.arrayContaining([expect.objectContaining({ id: 'tilak', memberCount: 2 }), expect.objectContaining({ id: 'nehru', memberCount: 1 })]))
  })
  ...
  it('denies non-members and rejects unbounded or unsupported query inputs', async () => {
    ...
    await expect(outsider.call('listDirectoryMembers', { batchId })).rejects.toMatchObject({ code: 'functions/permission-denied' })
    ...
    await expect(active.call('listDirectoryMembers', { batchId, limit: 51 })).rejects.toMatchObject({ code: 'functions/invalid-argument' })
    await expect(active.call('listDirectoryMembers', { batchId, filters: { privateField: 'x' } })).rejects.toMatchObject({ code: 'functions/invalid-argument' })
    await expect(active.call('listDirectoryMembers', { batchId, filters: { city: 'pune', profession: 'engineer' } })).rejects.toMatchObject({ code: 'functions/invalid-argument' })
  })
})
