...
export function ProfilePage() {
  const { user } = useAuth(); const [profile, setProfile] = useState<Profile>({}); const [notice, setNotice] = useState('')
  ...
  return <section className="page-stack"><div><p className="eyebrow">Account</p><h1>Your profile</h1><p className="muted">Your profile is visible only to approved batch members.</p></div><form className="panel form-stack" key={JSON.stringify(profile)} onSubmit={save}>{fields.map(([key, label, max]) => <label key={key}>{label}{key === 'about' || key === 'favouriteSchoolMemory' ? <textarea name={key} defaultValue={profile[key]} maxLength={max} /> : <input name={key} defaultValue={profile[key]} maxLength={max} required={key === 'displayName'} />}</label>)}<button type="submit">Save profile</button>{notice && <p role="status">{notice}</p>}</form></section>
}
