...
type WorkspaceTab = 'requests' | 'reunion' | 'fund' | 'members' | 'announcements'
...
export function CoordinatorWorkspace({ onPermissionDenied }: { onPermissionDenied: () => Promise<void> }) {
  ...
  async function saveReunion(event: FormEvent<HTMLFormElement>) {
    ...
    await setDoc(doc(db, `batches/${PILOT_BATCH_ID}/reunion/config`), { status: data.get('status'), title: data.get('title'), venue: data.get('venue'), venueMapUrl: data.get('mapUrl'), accommodation: data.get('accommodation'), logistics: data.get('logistics'), instructions: data.get('instructions'), reunionStartDate: new Date(String(data.get('startDate'))), rsvpCutoffAt: new Date(String(data.get('cutoff'))), updatedAt: serverTimestamp() }, { merge: true })
  ...
  return <section className="page-stack"><div><p className="eyebrow">Coordinator</p><h1>Batch operations</h1></div>
    <nav className="workspace-tabs" aria-label="Coordinator sections">{[...WORKSPACE_TABS].map((item) => <button key={item.id} aria-pressed={tab === item.id} className={tab === item.id ? 'active' : ''} onClick={() => setTab(item.id)}>{item.label}</button>)}</nav>
    {tab === 'requests' && <section className="panel"><h2>Pending access requests</h2>{requests.map((request) => <div className="member-row" key={request.id}><span><strong>{request.displayName}</strong><small>{request.houseId ?? 'No house selected'}</small></span><span className="member-actions"><button onClick={() => void approve(request.id)}>Approve</button><button onClick={(event) => openDialog(event, { kind: 'reject', requestId: request.id })}>Reject</button></span></div>)}{hasMore.requests && <button type="button" onClick={() => void loadMore('requests')}>Load more requests</button>}</section>}
    {tab === 'announcements' && <form className="panel form-stack" onSubmit={announce}><h2>Batch announcement</h2><label>Title<input name="title" maxLength={120} required /></label><label>Message<textarea name="body" maxLength={1000} required /></label><button>Publish announcement</button></form>}
    {tab === 'reunion' && <><form className="panel form-stack" onSubmit={saveReunion}><h2>Reunion details</h2><label>Member-facing state<select name="status" defaultValue="announced"><option value="announced">Announced</option><option value="rsvp_open">RSVP open</option><option value="rsvp_closed">RSVP closed</option><option value="confirmed">Confirmed</option><option value="completed">Completed</option><option value="archived">Archived</option></select></label><label>Title<input name="title" defaultValue="Silver Jubilee Reunion" required /></label><label>Venue<input name="venue" required /></label><label>Google Maps link<input name="mapUrl" type="url" required /></label><label>Reunion starts<input name="startDate" type="datetime-local" required /></label><label>RSVP cutoff<input name="cutoff" type="datetime-local" required /></label><label>Accommodation<textarea name="accommodation" maxLength={3000} /></label><label>Arrival, parking and logistics<textarea name="logistics" maxLength={3000} /></label><label>Instructions<textarea name="instructions" maxLength={3000} /></label><button>Save reunion details</button></form>
