...
export function FinancePage() {
  ...
  const [config, setConfig] = useState<Config>({}); const [summary, setSummary] = useState<Summary>({}); const [expenses, setExpenses] = useState<Expense[]>([]); const [expenseCursor, setExpenseCursor] = useState<QueryDocumentSnapshot<DocumentData>>(); const [hasMoreExpenses, setHasMoreExpenses] = useState(false); const [notice, setNotice] = useState(''); const [showQr, setShowQr] = useState(false); const [qrUrl, setQrUrl] = useState('')
  useEffect(() => {
    const db = firebaseServices().db
    void Promise.all([getDoc(doc(db, `batches/${PILOT_BATCH_ID}/paymentConfig/current`)), getDoc(doc(db, `batches/${PILOT_BATCH_ID}/fundSummary/public`)), getDocs(query(collection(db, `batches/${PILOT_BATCH_ID}/expenses`), where('status', '==', 'approved'), orderBy('expenseDate', 'desc'), limit(PAGE_SIZE)))]).then(([payment, fund, expenseDocs]) => {
      setConfig(payment.data() ?? {})
      setSummary(fund.data() ?? {})
      setExpenses(expenseDocs.docs.map((item) => ({ id: item.id, ...item.data() } as Expense))); setExpenseCursor(expenseDocs.docs.at(-1)); setHasMoreExpenses(expenseDocs.size === PAGE_SIZE)
    })
  }, [])
  async function loadMoreExpenses() { if (!expenseCursor) return; const docs = await getDocs(query(collection(firebaseServices().db, `batches/${PILOT_BATCH_ID}/expenses`), where('status', '==', 'approved'), orderBy('expenseDate', 'desc'), startAfter(expenseCursor), limit(PAGE_SIZE))); setExpenses((current) => [...current, ...docs.docs.map((item) => ({ id: item.id, ...item.data() } as Expense))]); setExpenseCursor(docs.docs.at(-1)); setHasMoreExpenses(docs.size === PAGE_SIZE) }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const paymentForm = event.currentTarget; const form = new FormData(paymentForm); const screenshot = form.get('screenshot') as File
    try {
      paymentRequestId.current ??= crypto.randomUUID()
      const result = await httpsCallable<{ batchId: string; requestId: string; amountPaise: number; utr: string; paymentDate: string; contributionHead: string }, { claimId: string }>(firebaseServices().functions, 'submitPaymentClaim')({ batchId: PILOT_BATCH_ID, requestId: paymentRequestId.current, amountPaise: Math.round(Number(form.get('amount')) * 100), utr: String(form.get('utr')), paymentDate: String(form.get('paymentDate')), contributionHead: String(form.get('contributionHead')) })
      ...
      paymentForm.reset(); paymentRequestId.current = null; setNotice('Payment claim submitted for Coordinator review.')
