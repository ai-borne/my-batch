...
type AuthState = { user: User | null; membership: Membership | null; loading: boolean; signIn: () => Promise<void>; signOut: () => Promise<void>; refreshMembership: () => Promise<void> }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, async (nextUser) => {
      setUser(nextUser)
      if (nextUser) {
        await setDoc(doc(db, 'users', nextUser.uid), { displayName: nextUser.displayName ?? '', email: nextUser.email ?? '', photoURL: nextUser.photoURL ?? null, updatedAt: serverTimestamp(), createdAt: serverTimestamp() }, { merge: true })
        setMembership(await fetchMembership(nextUser.uid))
      } else setMembership(null)
      setLoading(false)
    })
  }, [])
  const value = useMemo(() => ({ user, membership, loading, refreshMembership, signIn: async () => { await signInWithPopup(firebaseServices().auth, new GoogleAuthProvider()) }, signOut: async () => { await signOut(firebaseServices().auth) } }), [user, membership, loading])
