...
Array.from({ length: listFixtureCount }, (_, index) => index).forEach((index) => {
  ...
  batch.set(db.doc(`batches/${batchId}/notifications/${uid}/items/demo-notification-${item}`), { kind: 'announcement', title: `Synthetic notification ${item}`, body: 'Staging-only fixture.', isDemo: true, createdAt })
  ...
  batch.set(db.doc(`batches/${batchId}/paymentClaims/demo-claim-${item}`), { memberUid: uid, amountPaise: 10000 + index, contributionHead: 'reunion', status: 'verified', isDemo: true, createdAt, updatedAt: createdAt })
